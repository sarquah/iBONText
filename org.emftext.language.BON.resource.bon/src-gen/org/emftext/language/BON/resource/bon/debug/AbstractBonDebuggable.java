...
public abstract class AbstractBonDebuggable {
	
	/**
	 * The list of breakpoints, where each breakpoint is represented by its location
	 * (a string) and the line number
	 */
	private java.util.List<org.emftext.language.BON.resource.bon.util.BonPair<String, Integer>> lineBreakpoints = new java.util.ArrayList<org.emftext.language.BON.resource.bon.util.BonPair<String, Integer>>();
	...
	public void startEventSocket(int eventPort) {
		try {
			...
			new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().logError("Can't create socket connection while launching.", e);
		}
	}
	...
	public void stopEventSocket() {
		try {
			...
			new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().logError("Exception while closing socket.", e);
		}
	}
	
	public void sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes command, boolean sendOnlyInDebugMode, String... arguments) {
		if (isDebugMode() || !sendOnlyInDebugMode) {
			org.emftext.language.BON.resource.bon.debug.BonDebugMessage message = new org.emftext.language.BON.resource.bon.debug.BonDebugMessage(command, arguments);
	...
	public void evaluateLineBreakpoint(org.eclipse.emf.common.util.URI uri, int currentLine) {
		if (isDebugMode()) {
			...
			for (int i = 0; i < lineBreakpoints.size(); i++) {
				org.emftext.language.BON.resource.bon.util.BonPair<String, Integer> breakpointLocationAndLine = lineBreakpoints.get(i);
				...
				if (breakpointLine.intValue() == currentLine && breakpointLocation.equals(location)) {
					// suspending...
					setSuspend(true);
					sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.SUSPENDED, true, new String[] {"breakpoint", "" + currentLine});
					break;
				}
			}
			waitIfSuspended();
	...
	public void waitIfSuspended() {
		...
		sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.RESUMED, true);
	}
	
	public void addLineBreakpoint(String location, int line) {
		lineBreakpoints.add(new org.emftext.language.BON.resource.bon.util.BonPair<String, Integer>(location, new Integer(line)));
	}
	
	public void removeLineBreakpoint(String location, int line) {
		lineBreakpoints.remove(new org.emftext.language.BON.resource.bon.util.BonPair<String, Integer>(location, new Integer(line)));
	}
	...
	public void terminate() {
		sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.TERMINATED, false);
		stopEventSocket();
	}
	...
	public void startUpAndWait() {
		...
		if (isDebugMode()) {
			sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.STARTED, true);
			setSuspend(true);
