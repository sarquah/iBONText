 ...
 */
public class BonDebuggableInterpreter<ResultType, ContextType> extends org.emftext.language.BON.resource.bon.debug.AbstractBonDebuggable {
	
	/**
	 * The actual interpreter. Interpretation is delegated to this object.
	 */
	private org.emftext.language.BON.resource.bon.util.AbstractBonInterpreter<ResultType, ContextType> interpreterDelegate;
	
	/**
	 * To check whether we must stop the execution after step over/into/return, we use
	 * a closure
	 */
	private org.emftext.language.BON.resource.bon.IBonCommand<org.eclipse.emf.ecore.EObject> stopCondition;
	...
	public BonDebuggableInterpreter(org.emftext.language.BON.resource.bon.util.AbstractBonInterpreter<ResultType, ContextType> interpreterDelegate, int eventPort) {
		this.eventPort = eventPort;
		this.interpreterDelegate = interpreterDelegate;
		this.interpreterDelegate.addListener(new org.emftext.language.BON.resource.bon.IBonInterpreterListener() {
	...
	public String[] getStack() {
		...
		for (org.eclipse.emf.ecore.EObject parent : parents) {
			String serializedStackElement = org.emftext.language.BON.resource.bon.util.BonStringUtil.encode(',', new String[] {parent.eClass().getName(), Integer.toString(stackFrameID), parent.eResource().getURI().toString(), Integer.toString(getLine(parent)), Integer.toString(getCharStart(parent)), Integer.toString(getCharEnd(parent))});
	...
	public org.emftext.language.BON.resource.bon.util.AbstractBonInterpreter<ResultType, ContextType> getInterpreterDelegate() {
		return interpreterDelegate;
	}
	
	private int getLine(org.eclipse.emf.ecore.EObject element) {
		int line = -1;
		org.emftext.language.BON.resource.bon.IBonLocationMap locationMap = getLocationMap(element);
		if (locationMap != null) {
	...
	private org.emftext.language.BON.resource.bon.IBonLocationMap getLocationMap(org.eclipse.emf.ecore.EObject element) {
	...
	private void evaluateStep(org.eclipse.emf.ecore.EObject element) {
		...
		org.emftext.language.BON.resource.bon.IBonCommand<org.eclipse.emf.ecore.EObject> stopCheck = stopCondition;
		if (stopCheck != null && stopCheck.execute(element)) {
			stopCondition = null;
			// suspending after step...
			setSuspend(true);
			sendEvent(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.SUSPENDED, true);
			return;
		}
		waitIfSuspended();
	}
	...
	public void stepOver() {
		...
		final int currentLevel = org.emftext.language.BON.resource.bon.util.BonEObjectUtil.getDepth(current);
		stopCondition = new org.emftext.language.BON.resource.bon.IBonCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
				...
				int depth = org.emftext.language.BON.resource.bon.util.BonEObjectUtil.getDepth(element);
	...
	public void stepReturn() {
		...
		final int parentLevel = org.emftext.language.BON.resource.bon.util.BonEObjectUtil.getDepth(current) - 1;
		stopCondition = new org.emftext.language.BON.resource.bon.IBonCommand<org.eclipse.emf.ecore.EObject>() {
			public boolean execute(org.eclipse.emf.ecore.EObject element) {
