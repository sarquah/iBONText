...
public class BonEclipseProxy {
	...
	public void getDefaultLoadOptionProviderExtensions(java.util.Map<Object, Object> optionsMap) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			...
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.emftext.language.BON.resource.bon.mopp.BonPlugin.EP_DEFAULT_LOAD_OPTIONS_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
					org.emftext.language.BON.resource.bon.IBonOptionProvider provider = (org.emftext.language.BON.resource.bon.IBonOptionProvider) element.createExecutableExtension("class");
					final java.util.Map<?, ?> options = provider.getOptions();
					final java.util.Collection<?> keys = options.keySet();
					for (Object key : keys) {
						org.emftext.language.BON.resource.bon.util.BonMapUtil.putAndMergeKeys(optionsMap, key, options.get(key));
					}
				} catch (org.eclipse.core.runtime.CoreException ce) {
					new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().logError("Exception while getting default options.", ce);
				}
			}
		}
	}
	...
	public void getResourceFactoryExtensions(java.util.Map<String, org.eclipse.emf.ecore.resource.Resource.Factory> factories) {
		if (org.eclipse.core.runtime.Platform.isRunning()) {
			org.eclipse.core.runtime.IExtensionRegistry extensionRegistry = org.eclipse.core.runtime.Platform.getExtensionRegistry();
			org.eclipse.core.runtime.IConfigurationElement configurationElements[] = extensionRegistry.getConfigurationElementsFor(org.emftext.language.BON.resource.bon.mopp.BonPlugin.EP_ADDITIONAL_EXTENSION_PARSER_ID);
			for (org.eclipse.core.runtime.IConfigurationElement element : configurationElements) {
				try {
	 ...
	 */
	public org.emftext.language.BON.resource.bon.mopp.BonResource getResource(org.eclipse.core.resources.IFile file) {
	...
	public void checkEMFValidationConstraints(org.emftext.language.BON.resource.bon.IBonTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().isEclipsePlatformRunning()) {
			// The EMF validation framework code throws a NPE if the validation plug-in is not
			...
			if (org.eclipse.emf.validation.internal.EMFModelValidationPlugin.getPlugin() != null) {
				try {
					...
					new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().logError("Exception while checking contraints provided by EMF validator classes.", t);
				}
			}
		}
	}
	
	public void addStatus(org.eclipse.core.runtime.IStatus status, org.emftext.language.BON.resource.bon.IBonTextResource resource, org.eclipse.emf.ecore.EObject root) {
		...
		if (!status.isMultiStatus() || !hasChildren) {
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.ERROR) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addError(status.getMessage(), org.emftext.language.BON.resource.bon.BonEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
			if (status.getSeverity() == org.eclipse.core.runtime.IStatus.WARNING) {
				for (org.eclipse.emf.ecore.EObject cause : causes) {
					resource.addWarning(status.getMessage(), org.emftext.language.BON.resource.bon.BonEProblemType.ANALYSIS_PROBLEM, cause);
				}
			}
		}
		...
		if (uri != null && uri.isPlatform()) {
			...
			if (platformResource instanceof org.eclipse.core.resources.IFile) {
				...
				try {
					...
					new org.emftext.language.BON.resource.bon.util.BonRuntimeUtil().logWarning("Could not determine encoding of platform resource: " + uri.toString(), ce);
				}
			}
		}
		return null;
