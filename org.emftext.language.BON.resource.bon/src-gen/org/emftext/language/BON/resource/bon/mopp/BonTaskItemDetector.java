...
/**
 * The BonTaskItemDetector is used to find task items in text documents. The
 * current implementation searches for specific keywords to detect task items. The
 * BonTaskItemDetector is used both by the TaskItemBuilder and the editor.
 */
public class BonTaskItemDetector {
	
	public static String[] TASK_ITEM_KEYWORDS = new String[] {"TODO", "FIXME", "XXX"};
	
	public java.util.List<org.emftext.language.BON.resource.bon.mopp.BonTaskItem> findTaskItems(String text, int line, int charStart) {
		java.util.List<org.emftext.language.BON.resource.bon.mopp.BonTaskItem> foundItems = new java.util.ArrayList<org.emftext.language.BON.resource.bon.mopp.BonTaskItem>();
		...
		while (remainingText != null && continueSearch) {
			...
			for (String keyword : TASK_ITEM_KEYWORDS) {
				...
				if (index >= 0) {
					...
					foundItems.add(new org.emftext.language.BON.resource.bon.mopp.BonTaskItem(keyword, message, localLine, offset, end));
