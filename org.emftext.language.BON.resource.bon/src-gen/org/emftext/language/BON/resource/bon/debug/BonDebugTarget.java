...
public class BonDebugTarget extends org.emftext.language.BON.resource.bon.debug.BonDebugElement implements org.eclipse.debug.core.model.IDebugTarget, org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener {
	
	/**
	 * associated system process
	 */
	private org.emftext.language.BON.resource.bon.debug.BonDebugProcess process;
	...
	private org.emftext.language.BON.resource.bon.debug.BonDebugThread thread;
	 ...
	 * event listeners
	 */
	private java.util.List<org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener> eventListeners = new java.util.ArrayList<org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener>();
	
	private org.emftext.language.BON.resource.bon.debug.BonDebugProxy debugProxy;
	...
	private class DebugEventDispatcher extends org.eclipse.core.runtime.jobs.Job {
		...
		public DebugEventDispatcher() {
			super(new org.emftext.language.BON.resource.bon.mopp.BonMetaInformation().getSyntaxName() + " Event Dispatch");
			setSystem(true);
		}
		
		protected org.eclipse.core.runtime.IStatus run(org.eclipse.core.runtime.IProgressMonitor monitor) {
			while (!isTerminated()) {
				org.emftext.language.BON.resource.bon.debug.BonDebugMessage message = communicationHelper.receive(eventReader);
				if (message != null) {
		...
		private void notifyListeners(org.emftext.language.BON.resource.bon.debug.BonDebugMessage message) {
			Object[] listeners = eventListeners.toArray();
			for (int i = 0; i < listeners.length; i++) {
				((org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener) listeners[i]).handleMessage(message);
			}
		}
	}
	
	public BonDebugTarget(org.emftext.language.BON.resource.bon.debug.BonDebugProcess process, org.eclipse.debug.core.ILaunch launch, int requestPort, int eventPort) {
		super(launch.getDebugTarget());
		this.process = process;
		this.launch = launch;
		this.thread = new org.emftext.language.BON.resource.bon.debug.BonDebugThread(this);
		this.threads = new org.eclipse.debug.core.model.IThread[] {this.thread};
		
		// initialize debug proxy
		try {
			this.debugProxy = new org.emftext.language.BON.resource.bon.debug.BonDebugProxy(this, requestPort);
	 ...
	 */
	public void addEventListener(org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener listener) {
	 ...
	 * @param listener event listener
	 */
	public void removeEventListener(org.emftext.language.BON.resource.bon.debug.IBonDebugEventListener listener) {
		eventListeners.remove(listener);
	}
	
	public String getName() throws org.eclipse.debug.core.DebugException {
		return new org.emftext.language.BON.resource.bon.mopp.BonMetaInformation().getSyntaxName() + " model";
	}
		...
		if (supportsBreakpoint(breakpoint)) {
			try {
				if ((breakpoint.isEnabled() && getBreakpointManager().isEnabled()) || !breakpoint.isRegistered()) {
					org.emftext.language.BON.resource.bon.debug.BonLineBreakpoint lineBreakpoint = (org.emftext.language.BON.resource.bon.debug.BonLineBreakpoint) breakpoint;
	...
	public void handleMessage(org.emftext.language.BON.resource.bon.debug.BonDebugMessage message) {
		try {
			if (message.hasType(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.STARTED)) {
				started();
			} else if (message.hasType(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.SUSPENDED)) {
				suspend();
			} else if (message.hasType(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.TERMINATED)) {
				terminated();
			} else if (message.hasType(org.emftext.language.BON.resource.bon.debug.EBonDebugMessageTypes.RESUMED)) {
	...
	public org.emftext.language.BON.resource.bon.debug.BonDebugProxy getDebugProxy() {
