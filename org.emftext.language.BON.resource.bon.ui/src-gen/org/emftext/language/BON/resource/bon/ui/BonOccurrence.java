 ...
 */
public class BonOccurrence {
	
	public final static String OCCURRENCE_ANNOTATION_ID = "org.emftext.language.BON.resource.bon.ui.occurences";
	public final static String DECLARATION_ANNOTATION_ID = "org.emftext.language.BON.resource.bon.ui.occurences.declaration";
	...
	private org.emftext.language.BON.resource.bon.ui.BonTokenScanner tokenScanner;
	...
	private org.emftext.language.BON.resource.bon.IBonTextResource textResource;
	 ...
	 */
	public BonOccurrence(org.emftext.language.BON.resource.bon.IBonTextResource textResource, org.eclipse.jface.text.source.projection.ProjectionViewer sourceViewer, org.emftext.language.BON.resource.bon.ui.BonTokenScanner tokenScanner) {
		this.textResource = textResource;
		this.projectionViewer = sourceViewer;
		
		quotedTokenArray = new java.util.ArrayList<String>();
		String[] tokenNames = new org.emftext.language.BON.resource.bon.mopp.BonMetaInformation().getTokenNames();
	 ...
	 */
	public void handleOccurrenceHighlighting(org.emftext.language.BON.resource.bon.ui.BonBracketSet bracketSet) {
		if (textResource == null) {
			return;
		}
		org.eclipse.swt.custom.StyledText textWidget = projectionViewer.getTextWidget();
		...
		if (resource.equals(textResource)) {
			...
			while (!token.isEOF()) {
				...
				if (text.equals(tokenText)) {
					...
					addAnnotation(document, org.emftext.language.BON.resource.bon.ui.BonPositionCategory.DEFINTION, text);
					break;
				}
				token = tokenScanner.nextToken();
			}
			...
			if (text != null && text.equals(tokenText) && tokenScanner.getTokenOffset() != defPosition) {
				...
				if (occEO != null) {
					if (elementsAtDefinition.contains(occEO) || definitionElement.equals(occEO)) {
						addAnnotation(document, org.emftext.language.BON.resource.bon.ui.BonPositionCategory.PROXY, text);
					}
				}
			}
			token = tokenScanner.nextToken();
	...
	private void addAnnotation(org.eclipse.jface.text.IDocument document, org.emftext.language.BON.resource.bon.ui.BonPositionCategory type, String text) {
		int tokenOffset = tokenScanner.getTokenOffset();
		int tokenLength = tokenScanner.getTokenLength();
		...
		if (type == org.emftext.language.BON.resource.bon.ui.BonPositionCategory.DEFINTION) {
	...
	private void removeAnnotations() {
		removeAnnotations(org.emftext.language.BON.resource.bon.ui.BonOccurrence.OCCURRENCE_ANNOTATION_ID);
		removeAnnotations(org.emftext.language.BON.resource.bon.ui.BonOccurrence.DECLARATION_ANNOTATION_ID);
	}
